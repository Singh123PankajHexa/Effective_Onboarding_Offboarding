  ...
  return (
    <Box sx={{ py: 1.25, borderTop: 1, borderColor: 'divider', '&:first-of-type': { borderTop: 0, pt: 0.5 } }}>
      ...
      <Typography variant="caption" color={`${color}.main`} sx={{ display: 'block', mt: 0.5 }}>
        {completed}/{total} items cleared &middot; target {target}% &middot; {met ? 'on target' : 'below target'}
      </Typography>
    </Box>
  );
}

export default function DepartmentClearance({ departments }) {
  return (
    <Card variant="outlined">
      <CardContent>
        ...
        <Typography variant="caption" color="text.secondary">
          Clearance completion vs. SLA target, by department
        </Typography>
        {departments.map((d) => (
